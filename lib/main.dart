...
class _MapScreenState
  ...
  void didUpdateWidget(
    ...
    if (widget.corridorActive &&
        ...
        widget.activeSignalIndex <
            widget.trafficSignals.length) {
      final index = widget.activeSignalIndex;
  ...
  Future<void> loadSignalRoute(
    ...
    final signal = signalLocation(index);
    final hospital = hospitalLocation;

    try {
      // Snap the selected signal to the nearest road.
      final nearestUrl = Uri.parse(
        'https://router.project-osrm.org/nearest/v1/driving/'
        '${signal.longitude},${signal.latitude}'
        '?number=1',
      );

      final nearestResponse = await http
          .get(nearestUrl)
          .timeout(
            const Duration(seconds: 30),
          );

      LatLng routeStart = signal;

      if (nearestResponse.statusCode == 200) {
        final nearestData =
            jsonDecode(nearestResponse.body);

        if (nearestData['code'] == 'Ok') {
          final waypoints =
              nearestData['waypoints'] as List<dynamic>;

          if (waypoints.isNotEmpty) {
            final location =
                waypoints.first['location'] as List<dynamic>;
            ...
            // Move marker to the exact route starting point.
            widget.trafficSignals[index]['lat'] =
                routeStart.latitude;
            widget.trafficSignals[index]['lng'] =
      ...
      if (response.statusCode != 200) {
        throw Exception('Signal route error');
      }

      final data = jsonDecode(response.body);

      if (data['code'] != 'Ok') {
        throw Exception('No signal route');
      }
      ...
      final geometry =
          Map<String, dynamic>.from(route['geometry']);
      ...
      setState(() {
        ...
        signalRouteDistances[index] = distance;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Signal route unavailable.'),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final List<LatLng> displayRoute = signalRoutes[displaySignalIndex] ?? <LatLng>[];
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            showSignalRoute
                                ? 'Signal ${displaySignalIndex + 1} → Hospital road path'
                                : 'Ambulance → Hospital road route',
                            style:
                                const TextStyle(
                              fontSize: 12,
                            ),
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            showSignalRoute
                                ? 'Signal ${displaySignalIndex + 1} → Hospital'
